...
export const MODELS = {
  ...
  DALLE2: "dall-e-2",
  DALLE3: "dall-e-3",
  GEMINI25: "gemini-2.5-flash-image-preview",
} as const;
...
export type ImageResponseFormat =
  (typeof RESPONSE_FORMATS)[keyof typeof RESPONSE_FORMATS];
...
export type ImageOutputFormat =
  (typeof OUTPUT_FORMATS)[keyof typeof OUTPUT_FORMATS];
export type ImageModeration =
  (typeof MODERATION_LEVELS)[keyof typeof MODERATION_LEVELS];
...
export class OpenAIImageClient {
  ...
  constructor(
    ...
    // Support for custom base URL via constructor or environment variable
    const effectiveBaseUrl =
      customBaseUrl ||
      process.env.OPENAI_BASE_URL ||
      "https://api.openai.com/v1";
    this.baseUrl = `${effectiveBaseUrl.replace(/\/$/, "")}/images/generations`;
    this.editUrl = `${effectiveBaseUrl.replace(/\/$/, "")}/images/edits`;

    // If specific models are provided, use them (including custom models)
    if (allowedModels && allowedModels.length > 0) {
      this.allowedModels = {};
      allowedModels.forEach((modelName, index) => {
        // Check if it's a predefined model
        const modelEntry = Object.entries(MODELS).find(
          ([_, value]) => value === modelName,
        );
        if (modelEntry) {
          const [key, value] = modelEntry;
          this.allowedModels[key] = value;
        } else {
          // For custom models (like gemini-2.5-flash-image-preview), create a dynamic entry
          console.log(`Adding custom model: "${modelName}"`);
          this.allowedModels[`CUSTOM_${index}`] = modelName as any;
        }
      });
    } else {
  ...
  async editImages(params: ImageEditRequest): Promise<ImageGenerationResponse> {
    ...
    if (params.model) {
      const isModelAllowed = Object.values(this.allowedModels).includes(
        params.model,
      );
      if (!isModelAllowed) {
        throw new Error(
          `Model "${params.model}" is not allowed. Allowed models: ${Object.values(this.allowedModels).join(", ")}`,
        );
      }

      // Check if model is supported for edits
      // Note: Custom models like gemini-2.5-flash-image-preview also support image editing
    }
    ...
    if (
      ...
      throw new Error(
        "dall-e-2 only supports sizes 256x256, 512x512, or 1024x1024",
      );
    }
    ...
    if (params.mask) {
      ...
      formData.append("mask", maskBuffer, {
        filename: path.basename(params.mask),
      });
    }
  ...
  async generateImages(
    ...
    if (
      ...
      throw new Error(
        "dall-e-3 only supports sizes 1024x1024, 1792x1024, or 1024x1792",
      );
    }

    // Handle transparency requirements
    if (
      params.background === BACKGROUNDS.TRANSPARENT &&
      params.output_format &&
      params.output_format !== "png" &&
      params.output_format !== "webp"
    ) {
      throw new Error(
        "When background is transparent, output_format must be png or webp",
      );
    }
  ...
  saveResponseImageToTempFile(
    ...
    return this.saveImageToTempFile(
      response.data[index].b64_json,
      outputFormat,
      outputPath,
    );
  }
